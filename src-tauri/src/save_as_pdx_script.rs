...
pub fn save_as_pdx_script(event: WindowMenuEvent) {
  ...
  let mut game_states: Vec<State> = vec![];

  states.iter().for_each(|state| {
    let current_state = current_state_map.get(&state.name).unwrap();

    if compare_sub_states(&state.sub_states, current_state) {
      current_state_map.remove(&state.name);
    } else {
      game_states.push(state.clone());
    }
  });
...
fn compare_sub_states(state1: &[SubState], state2: &[SubState]) -> bool {
