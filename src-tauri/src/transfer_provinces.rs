...
pub fn transfer_province (state: &str, province: &str, from_country: Country, to_country: Country, from_coords: Vec<Vec<(f32, f32)>>, to_coords: Vec<Vec<(f32, f32)>>, province_coords: Vec<Vec<(f32, f32)>>) -> TransferProvinceResponse {
  ...
  let (new_from_country, pops_given) = remove_province_from_country(from_country.clone(), state, province, &province_coords);
  let new_to_country = add_province_to_country(to_country.clone(), state, province, &province_coords, pops_given);
...
fn add_province_to_country(mut country: Country, state: &str, province: &str, province_coords: &MultiPolygon<f32>, pops_given: Vec<Pop>) -> Country {
...
fn remove_province_from_country(mut country: Country, state: &str, province: &str, province_coords: &MultiPolygon<f32>) -> (Country, Vec<Pop>) {
  ...
  let pops_given = match new_provinces.len() > 0 {
    true => {
      country.states.push(State {
        name: state.to_string(),
        provinces: new_provinces,
