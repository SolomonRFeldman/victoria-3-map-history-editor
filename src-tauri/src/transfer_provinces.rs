...
pub fn transfer_province(
    ...
    let (new_from_country, pops_given, new_state_buildings) =
        remove_province_from_country(from_country.clone(), state, province, &province_coords);
    let new_to_country = add_province_to_country(
        to_country.clone(),
        state,
        province,
        &province_coords,
...
fn add_province_to_country(
    ...
    let existing_state = country
        .states
        .iter()
        .find(|to_state| to_state.name == state);
    ...
    country.coordinates =
        multi_poly_to_vec(vec_to_multi_poly(country.coordinates).union(province_coords));
    country
}

fn remove_province_from_country(
    mut country: Country,
    state: &str,
    province: &str,
    province_coords: &MultiPolygon<f32>,
    ...
    country.coordinates =
        multi_poly_to_vec(vec_to_multi_poly(country.coordinates).difference(province_coords));
