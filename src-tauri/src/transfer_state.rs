...
pub fn transfer_state(
    ...
    let new_to_country =
        add_state_to_country(to_country.clone(), &from_country, state, &from_coords);
...
fn add_state_to_country(
    ...
    country.coordinates =
        multi_poly_to_vec(vec_to_multi_poly(country.coordinates).union(to_coords));
...
fn remove_state_from_country(
    mut country: Country,
    state: &str,
    coords: &MultiPolygon<f32>,
) -> Country {
    country.states.retain(|from_state| from_state.name != state);
    country.coordinates =
        multi_poly_to_vec(vec_to_multi_poly(country.coordinates).difference(coords));
