...
impl MainMenu {
  pub fn create_menu() -> Menu {
    ...
    let open_working_directory = CustomMenuItem::new("open-working-directory", "Open Working Directory");
    let save = CustomMenuItem::new(SAVE, "Save");
    let exit = CustomMenuItem::new(EXIT, "Exit");
    let submenu = Submenu::new("File", Menu::new().add_item(open_game_folder).add_item(open_working_directory).add_item(save).add_item(exit));
...
fn handle_open_working_directory(event: WindowMenuEvent) {
  FileDialogBuilder::new().pick_folder(move |file_path| {
    if let Some(file_path) = file_path {
      let config_path = event.window().app_handle().path_resolver().app_cache_dir().unwrap().join("config.json");
